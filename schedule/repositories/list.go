...
func (p *schedulePaymentRepositories) ListPlans(userId uuid.UUID) ([]schedule_payment_model.SchedulePaymentPlan, error) {

	ctx, cancel := context.WithTimeout(context.Background(), util.DbTimeout)
	defer cancel()
	stmt := `Select id, user_id, account_id ,amount,periods,paid_periods,paydate,duration, is_completed,created_at, updated_at from scheduled_payments where is_completed <> true and (paydate  BETWEEN NOW() - INTERVAL '6 hours' AND NOW())`
