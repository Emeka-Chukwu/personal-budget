...
func (repo *accountRepository) Update(model model_account.Account) (*model_account.Account, error) {
	...
	err := repo.DB.QueryRowContext(ctx, stmt, model.Name, model.Number).
		Scan(
			&resp.ID,
			&resp.Name,
			&resp.Number,
